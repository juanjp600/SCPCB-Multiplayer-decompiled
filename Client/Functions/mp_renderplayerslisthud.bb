Function mp_renderplayerslisthud%(arg0%)
    ...
    If ((((((ismultiplayerhudenabled Lor menuopen) Lor consoleopen) = $00) And keydown($19)) Lor mp_playerslist\Field0) <> 0) Then
        ...
        For local3 = Each mp_player
            local2 = (((30.0 * menuscale) + local2) - (4.0 * menuscale))
            local0 = (local0 + $01)
        Next
        local4 = (Float (opt\Field49 Sar $01))
        local5 = ((Float (opt\Field50 Sar $01)) + (15.0 * menuscale))
        color($FF, $FF, $FF)
        textex((Int local4), (Int ((local5 - (local2 / 2.0)) + (12.0 * menuscale))), (((((ue_server\Field4 + " (") + (Str local0)) + " / ") + (Str ue_server\Field8)) + ")"), $01, $00)
    EndIf
    Return $00
End Function
