Function updatemtf%()
    ...
    Local local6.npcs
    Local local7.items
    If (((playerroom\Field7\Field11 = "gateaentrance") Or (networkserver\Field18 = $00)) <> 0) Then
        Return $00
    EndIf
    If (0.0 = mtftimer) Then
        If (rand($1E, $01) = $01) Then
            ...
            For local0 = Each rooms
                If (local0\Field7\Field11 = "gateaentrance") Then
                    local4 = local0
                    Exit
                EndIf
            Next
            If (local4 <> Null) Then
                For local5 = Each players
                    If (((((0.0 <> local5\Field1) Or (0.0 <> local5\Field2)) Or (0.0 <> local5\Field3)) And local5\Field43) <> 0) Then
                        If (((local5\Field46 <> "pocketdimension") And (local5\Field46 <> "dimension1499")) <> 0) Then
                            If (30.0 > (Abs (entityz(local4\Field2, $00) - entityz(local5\Field13, $00)))) Then
                                mtftimer = fpsfactor
                                playannouncement("SFX\Character\MTF\Announc.ogg", $01, $00)
    ...
    ElseIf (((8400.0 < mtftimer) And (10000.0 > mtftimer)) <> 0) Then
        playannouncement("SFX\Character\MTF\AnnouncAfter1.ogg", $01, $00)
        mtftimer = 10000.0
    ElseIf (((10000.0 <= mtftimer) And (18400.0 >= mtftimer)) <> 0) Then
        mtftimer = (mtftimer + fpsfactor)
    ElseIf (((18400.0 < mtftimer) And (20000.0 > mtftimer)) <> 0) Then
        playannouncement("SFX\Character\MTF\AnnouncAfter2.ogg", $01, $00)
        mtftimer = 20000.0
        ...
        For local7 = Each items
            If (local7\Field22 <> $00) Then
                If ((((left(local7\Field3\Field1, $04) = "SCP-") And (left(local7\Field3\Field1, $07) <> "SCP-035")) And (left(local7\Field3\Field1, $07) <> "SCP-093")) <> 0) Then
                    playannouncement("SFX\Character\MTF\ThreatAnnouncPossession.ogg", $01, $00)
                    mtftimer = 25000.0
                    Return $00
                    Exit
                EndIf
            EndIf
        Next
        playannouncement((("SFX\Character\MTF\ThreatAnnounc" + (Str rand($01, $03))) + ".ogg"), $01, $00)
        mtftimer = 25000.0
    ElseIf (((25000.0 <= mtftimer) And (29200.0 >= mtftimer)) <> 0) Then
        mtftimer = (mtftimer + fpsfactor)
    ElseIf (((29200.0 < mtftimer) And (30000.0 > mtftimer)) <> 0) Then
        playannouncement("SFX\Character\MTF\ThreatAnnouncFinal.ogg", $01, $00)
        mtftimer = 30000.0
    EndIf
    Return $00
End Function
